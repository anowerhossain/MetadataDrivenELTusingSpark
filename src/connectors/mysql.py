"""
MySQL Connection and JDBC Data Reader Module.
Resolves credentials from environment variables and reads data from MySQL into PySpark DataFrames.
"""
...
logger = setup_logger("MySQLConnector")


@dataclass
class MySQLConnectionConfig:
    """Holds validated MySQL connection properties safely without exposing secrets in logs."""
    connection_name: str
    jdbc_url: str
    username: str
    ...
    def __repr__(self) -> str:
        return (
            f"MySQLConnectionConfig(connection_name='{self.connection_name}', "
...
class MySQLConnectionResolver:
    """Resolves MySQL connection details from environment variables for a named connection."""

    @classmethod
    def resolve(cls, source_config: SourceSection) -> MySQLConnectionConfig:
        """Resolves MySQL connection credentials for the given SourceSection."""
        ...
        if not jdbc_url or not jdbc_url.strip():
            raise ConfigError(
                f"MySQL connection '{connection_name}' error: Missing environment variable '{prefix}_JDBC_URL' "
        ...
        if not username or not username.strip():
            raise ConfigError(
                f"MySQL connection '{connection_name}' error: Missing environment variable '{prefix}_USERNAME' "
                f"(or '{prefix}_USER')."
            )
        ...
        if not password or not password.strip():
            raise ConfigError(
                f"MySQL connection '{connection_name}' error: Missing environment variable '{prefix}_PASSWORD'."
            )

        # 4. Resolve Driver & Fetch Size
        driver = os.environ.get(f"{prefix}_DRIVER", "com.mysql.cj.jdbc.Driver")
        ...
        conn_config = MySQLConnectionConfig(
        ...
        logger.info(f"Resolved MySQL connection successfully: {conn_config}")
        return conn_config


class MySQLReader:
    """Reads data from MySQL tables into PySpark DataFrames via JDBC."""
    ...
    def get_full_table_name(self) -> str:
        """Formats full table name (e.g. 'sales_db.orders' or 'orders')."""
    ...
    def get_connection_config(self) -> MySQLConnectionConfig:
        """Resolves secure connection credentials."""
        return MySQLConnectionResolver.resolve(self.config)
    ...
    def read(
        ...
        """Executes extraction from MySQL database table based on load configuration."""
        ...
        if load_type == LoadType.INCREMENTAL:
            ...
            logger.info(
                f"Initializing MySQL JDBC INCREMENTAL extraction for table '{full_table}' "
            ...
            logger.info(
                f"Initializing MySQL JDBC FULL extraction for table '{full_table}' "
        ...
        try:
            ...
            logger.info(f"MySQL JDBC load completed successfully for table '{full_table}'.")
            return df
        except Exception as err:
            logger.error(f"Failed to extract data from MySQL table '{full_table}': {err}")
            raise RuntimeError(f"MySQL JDBC extraction failed for table '{full_table}': {err}") from err
