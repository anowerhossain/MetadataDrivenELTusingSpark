"""
Microsoft SQL Server Connection and JDBC Data Reader Module.
Resolves credentials from environment variables and reads data from SQL Server into PySpark DataFrames.
"""
...
logger = setup_logger("SQLServerConnector")


@dataclass
class SQLServerConnectionConfig:
    """Holds validated SQL Server connection properties safely without exposing secrets in logs."""
    connection_name: str
    jdbc_url: str
    username: str
    ...
    def __repr__(self) -> str:
        return (
            f"SQLServerConnectionConfig(connection_name='{self.connection_name}', "
...
class SQLServerConnectionResolver:
    """Resolves SQL Server connection details from environment variables for a named connection."""

    @classmethod
    def resolve(cls, source_config: SourceSection) -> SQLServerConnectionConfig:
        """Resolves SQL Server connection credentials for the given SourceSection."""
        ...
        if not jdbc_url:
            ...
            database = os.environ.get(f"{prefix}_DATABASE") or os.environ.get(f"{prefix}_DB") or source_config.schema
            if host and database:
                jdbc_url = f"jdbc:sqlserver://{host}:{port};databaseName={database};encrypt=false;"

        if not jdbc_url or not jdbc_url.strip():
            raise ConfigError(
                f"SQL Server connection '{connection_name}' error: Missing environment variable '{prefix}_JDBC_URL' "
                f"(or '{prefix}_HOST' and '{prefix}_DATABASE')."
            )
        ...
        if not username or not username.strip():
            raise ConfigError(
                f"SQL Server connection '{connection_name}' error: Missing environment variable '{prefix}_USERNAME' "
                f"(or '{prefix}_USER')."
            )
        ...
        if not password or not password.strip():
            raise ConfigError(
                f"SQL Server connection '{connection_name}' error: Missing environment variable '{prefix}_PASSWORD'."
            )

        # 4. Resolve Driver & Fetch Size
        driver = os.environ.get(f"{prefix}_DRIVER", "com.microsoft.sqlserver.jdbc.SQLServerDriver")
        ...
        logger.info(f"Resolved SQL Server connection successfully: {conn_config}")
        return conn_config


class SQLServerReader:
    """Reads data from Microsoft SQL Server tables into PySpark DataFrames via JDBC."""
    ...
    def get_full_table_name(self) -> str:
        """Formats full table name (e.g. 'dbo.invoices' or 'invoices')."""
        schema_str = self.config.schema.strip() if self.config.schema else "dbo"
    ...
    def get_connection_config(self) -> SQLServerConnectionConfig:
        """Resolves secure connection credentials."""
        return SQLServerConnectionResolver.resolve(self.config)
    ...
    def read(
        ...
        """Executes extraction from SQL Server database table based on load configuration."""
        ...
        if load_type == LoadType.INCREMENTAL:
            ...
            logger.info(
                f"Initializing SQL Server JDBC INCREMENTAL extraction for table '{full_table}' "
            ...
            logger.info(
                f"Initializing SQL Server JDBC FULL extraction for table '{full_table}' "
        ...
        try:
            ...
            logger.info(f"SQL Server JDBC load completed successfully for table '{full_table}'.")
            return df
        except Exception as err:
            logger.error(f"Failed to extract data from SQL Server table '{full_table}': {err}")
            raise RuntimeError(f"SQL Server JDBC extraction failed for table '{full_table}': {err}") from err
