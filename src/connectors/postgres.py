"""
PostgreSQL Connection and JDBC Data Reader Module.
Resolves credentials from environment variables and reads data from PostgreSQL into PySpark DataFrames.
"""
...
class PostgresConnectionConfig:
    """Holds validated PostgreSQL connection properties safely without exposing secrets in logs."""
    connection_name: str
    jdbc_url: str
    username: str
    ...
    def __repr__(self) -> str:
        return (
            f"PostgresConnectionConfig(connection_name='{self.connection_name}', "
...
class PostgresConnectionResolver:
    """Resolves PostgreSQL connection details from environment variables for a named connection."""

    @classmethod
    def resolve(cls, source_config: SourceSection) -> PostgresConnectionConfig:
        """Resolves PostgreSQL connection credentials for the given SourceSection."""
        ...
        if not jdbc_url or not jdbc_url.strip():
            raise ConfigError(
                f"PostgreSQL connection '{connection_name}' error: Missing environment variable '{prefix}_JDBC_URL' "
        ...
        if not username or not username.strip():
            raise ConfigError(
                f"PostgreSQL connection '{connection_name}' error: Missing environment variable '{prefix}_USERNAME' "
                f"(or '{prefix}_USER')."
            )
        ...
        if not password or not password.strip():
            raise ConfigError(
                f"PostgreSQL connection '{connection_name}' error: Missing environment variable '{prefix}_PASSWORD'."
            )

        # 4. Resolve Driver & Fetch Size
        driver = os.environ.get(f"{prefix}_DRIVER", "org.postgresql.Driver")
        ...
        logger.info(f"Resolved PostgreSQL connection successfully: {conn_config}")
        return conn_config


class PostgresReader:
    """Reads data from PostgreSQL tables into PySpark DataFrames via JDBC."""
    ...
    def get_full_table_name(self) -> str:
        """Formats full table name (e.g. 'public.payments' or 'payments')."""
        schema_str = self.config.schema.strip() if self.config.schema else "public"
    ...
    def get_connection_config(self) -> PostgresConnectionConfig:
        ...
        return PostgresConnectionResolver.resolve(self.config)
    ...
    def read(
        ...
        """Executes extraction from PostgreSQL database table based on load configuration."""
        ...
        if load_type == LoadType.INCREMENTAL:
            ...
            logger.info(
                f"Initializing PostgreSQL JDBC INCREMENTAL extraction for table '{full_table}' "
            ...
            logger.info(
                f"Initializing PostgreSQL JDBC FULL extraction for table '{full_table}' "
        ...
        try:
            ...
            logger.info(f"PostgreSQL JDBC load completed successfully for table '{full_table}'.")
            return df
        except Exception as err:
            logger.error(f"Failed to extract data from PostgreSQL table '{full_table}': {err}")
            raise RuntimeError(f"PostgreSQL JDBC extraction failed for table '{full_table}': {err}") from err
